...
Use `git_repository_github_cli` for GitHub and
`git_repository_gitlab_cli` for GitLab. Pass arguments without the `gh` or
`glab` executable name. On MCP hosts, pass the absolute coding workspace as
`cwd`.
