...
Name tools by role and backend rather than by a broad shell capability. This project keeps exactly 13 mappings across issue tracking, wiki, Git repository, and code review in the shared catalog. A declarative Runtime Tool Profile then binds exactly one issue-tracker, wiki, and Git backend for each adapter: Pi/Codex use Linear, Notion, and GitHub; Claude Code/OpenCode use Jira, Confluence, and GitLab. The adapter must filter both tool listing and tool execution, so a hidden backend cannot be reached by calling its catalog name directly.
...
Classify API body flags and write subcommands as mutations. A mutation requires both explicit user intent for that exact operation and `confirmedWrite=true`; this interlock applies equally to every runtime adapter. Keep installer behavior separate: external CLI installation is preview-first and exact npm versions are pinned, while Homebrew formulae remain explicitly reviewed manager commands. The selected runtime set derives the default union of executables, but an explicit installer override never changes adapter exposure.
