...
Name tools by role and backend rather than by a broad shell capability. This project keeps exactly 13 mappings across issue tracking, wiki, Git repository, and code review in the shared catalog. A schema-validated `runtime-tools.json` manifest defines reusable Runtime Tool Profiles and assigns each runtime by profile ID: Pi/Codex use the `personal` Linear, Notion, and GitHub profile; Claude Code/OpenCode use the `company` Jira, Confluence, and GitLab profile. The manifest is packaged inside the plugin snapshot so every adapter and `omh` can consume the same source. The adapter must filter both tool listing and tool execution, so a hidden backend cannot be reached by calling its catalog name directly.
...
Classify API body flags and write subcommands as mutations. A mutation requires both explicit user intent for that exact operation and `confirmedWrite=true`; this interlock applies equally to every runtime adapter. Keep installer behavior separate: external CLI installation is preview-first and exact npm versions are pinned, while Homebrew formulae remain explicitly reviewed manager commands. One `omh setup --apply` resolves the selected runtime assignments and installs their deduplicated profile union automatically. An explicit installer override never changes adapter exposure.

## Verification

Assert the full catalog and each runtime's exact three-tool selection, then test classifier bypasses, hidden direct calls, workspace-local executable shims, redaction, MCP list/call behavior, OpenCode imports, and Pi registration. Validate both plugin manifests and every shared skill. Finally load the Claude plugin from an arbitrary current working directory: plugin-root-relative MCP paths often work in a manifest validator but fail when the server actually starts.
