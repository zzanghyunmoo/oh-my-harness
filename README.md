...
- LiteLLM, Quotio, CCS의 secret-free proxy profile과 Quotio/CCS preview-first installer, CWD `.env` 자동 활성화
...
# 네 runtime, 외부 CLI, Quotio/CCS의 설치 계획만 확인
./omh setup
...
omh proxies install       Quotio/CCS 설치 preview/apply
omh proxies configure     준비된 endpoint/key를 CWD .env에 안전하게 활성화
omh proxies doctor        세 proxy의 /models 연결과 인증 확인
omh status                agent, CLI, proxy 상태를 한 번에 확인
...
- `harness/proxies`: LiteLLM/Quotio/CCS profile과 reviewed installer descriptor
...
- `scripts/proxies`: proxy 설치, CWD `.env` 설정, 연결 진단기
...
- `extensions/litellm-provider`, `extensions/quotio-provider`, `extensions/ccs-provider`: 각 proxy wire protocol에 맞춘 opt-in Pi providers
...
세 provider는 `harness/proxies/proxy-profiles.json`의 선언을 공유합니다. LiteLLM은 외부 배포 endpoint를 연결하고, Quotio와 CCS는 다음 명령의 기본 설치 대상입니다.

```bash
# 읽기 전용 preview
./omh proxies install

# macOS/Windows에서 지원되는 항목만 적용
./omh proxies install --apply
```

- LiteLLM: harness가 서버를 설치하지 않습니다. 별도로 배포한 gateway의 base URL과 virtual key를 사용합니다.
- Quotio `0.24.0`: 공식 DMG와 app executable의 reviewed SHA-256을 확인해 `~/.oh-my-harness/apps`에 설치합니다. 공식 지원 범위가 macOS 15+ Apple Silicon뿐이므로 Windows에서는 실패하지 않고 `unsupported`와 원격 endpoint 안내를 표시합니다.
- CCS `8.8.1`: macOS와 Windows 모두 exact `@kaitranntt/ccs` npm package로 설치하고, 설치 직전 registry의 `dist.integrity`가 reviewed SHA-512와 같은지 확인합니다. 설치 후 `ccs setup` 또는 `ccs config`에서 proxy를 준비합니다. CCS local proxy는 Anthropic-compatible wire protocol로 등록합니다.

endpoint와 API key가 발급되면 값 자체를 CLI 인자로 넘기지 않습니다. CWD `.env` 또는 현재 프로세스 환경에 해당 두 값을 준비한 뒤 configure를 실행하면, harness가 provider toggle과 값을 CWD `.env`에 원자적으로 저장하고 macOS/Linux에서는 권한을 `0600`으로 제한합니다. 출력과 JSON 결과에는 key가 포함되지 않습니다.
...
# 세 endpoint의 인증과 /models 응답 확인
./omh proxies doctor
```

Quotio의 일반적인 local base URL은 `http://localhost:8317/v1`이고, CCS도 기본 CLIProxy port `8317`을 사용할 수 있으므로 둘을 동시에 로컬에서 실행할 때는 서로 다른 port를 지정해야 합니다.
...
- `/litellm-status`, `/quotio-status`, `/ccs-status`: 각 proxy 연결과 인증 상태 확인
...
npm run proxies:install → omh proxies install
npm run proxies:configure → omh proxies configure
npm run proxies:doctor  → omh proxies doctor
