...
- macOS arm64와 Linux x64에서 Claude Code, Codex, OpenCode, Pi를 checksum으로 검증해 설치하고 각 런타임의 native package surface에 등록하는 preview-first installer
...
- npm
- Git
- Jira, GitHub, GitLab, CodeRabbit CLI를 자동 설치하려면 Homebrew
...
- `omh`, `bin/omh.mjs`: 사람을 위한 통합 preview-first 관리 CLI
...
외부 CLI는 harness package에 포함하지 않습니다. 아래 명령은 설치 상태를 검사하고, 빠진 CLI의 exact npm package 또는 검토된 Homebrew formula를 보여줍니다. 설치 명령도 기본값은 preview이며 `--apply`가 있을 때만 변경합니다.
